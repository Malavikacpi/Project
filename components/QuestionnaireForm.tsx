...
export default function QuestionnaireForm({ questionnaires }: { questionnaires: QuestionnaireCollection }) {
  ...
  return <main className="page-shell"><div className="app-frame">
    ...
    {view === "final" || view === "submitted" ? finalContent : view === "home" ? <section className="front-page">
      <div className="front-heading"><p>Questionnaire</p><h2>Select Power System</h2><span>Select the power system you would like to assess.</span></div>
      <div className="section-card-grid">
        <button className="section-card" onClick={() => openView("generation")}><span className="section-letter">G</span><div><small>Power system</small><strong>Generation</strong></div><span className="card-arrow">→</span></button>
        <button className="section-card" onClick={() => openView("transmission")}><span className="section-letter">T</span><div><small>Power system</small><strong>Transmission</strong></div><span className="card-arrow">→</span></button>
        <button className="section-card" onClick={() => openView("distribution")}><span className="section-letter">D</span><div><small>Power system</small><strong>Distribution</strong></div><span className="card-arrow">→</span></button>
      </div>
      ...
      <div className="asset-card-grid">{generationAssets.map((asset) => <button className="asset-card" onClick={() => selectAsset(asset.id)} key={asset.id}><span className="asset-icon">{asset.icon}</span><div><strong>{asset.title}</strong><small>Generation asset</small></div><span className="selection-mark">→</span></button>)}</div>
